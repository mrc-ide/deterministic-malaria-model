...
dim(MDA_A) <- c(na,nh,ni,ncc)
dim(MDA_T) <- c(na,nh,ni,ncc)
dim(MDA_D) <- c(na,nh,ni,ncc)
dim(MDA_U) <- c(na,nh,ni,ncc)
dim(MDA_S) <- c(na,nh,ni,ncc)

# MDA_A[1:na, 1:nh, 1:ni] = if(t == MDA_sr ) log(1/(1-MDA_cov))*A[i,j,k] else 0
# MDA_T[1:na, 1:nh, 1:ni] = if(t == MDA_sr ) log(1/(1-MDA_cov))*T[i,j,k] else 0
# MDA_D[1:na, 1:nh, 1:ni] = if(t == MDA_sr ) log(1/(1-MDA_cov))*D[i,j,k] else 0
# MDA_U[1:na, 1:nh, 1:ni] = if(t == MDA_sr ) log(1/(1-MDA_cov))*U[i,j,k] else 0
# MDA_S[1:na, 1:nh, 1:ni] = if(t == MDA_sr ) log(1/(1-MDA_cov))*S[i,j,k] else 0
...
MDA_A[1:na, 1:nh, 1:ni, 1:ncc] =  0
MDA_T[1:na, 1:nh, 1:ni, 1:ncc] =  0
MDA_D[1:na, 1:nh, 1:ni, 1:ncc] =  0
MDA_U[1:na, 1:nh, 1:ni, 1:ncc] =  0
MDA_S[1:na, 1:nh, 1:ni, 1:ncc] =  0

MDA_A[2:SMCyrs, 1:nh, 1:ni, 1] = if (t >= MDA_sr && t < (MDA_sr + 1)) log(1/(1-MDA_eff1))*A[i,j,k,l] else 0
MDA_T[2:SMCyrs, 1:nh, 1:ni, 1] = if (t >= MDA_sr && t < (MDA_sr + 1)) log(1/(1-MDA_eff1))*T[i,j,k,l] else 0
MDA_D[2:SMCyrs, 1:nh, 1:ni, 1] = if (t >= MDA_sr && t < (MDA_sr + 1)) log(1/(1-MDA_eff1))*D[i,j,k,l] else 0
MDA_U[2:SMCyrs, 1:nh, 1:ni, 1] = if (t >= MDA_sr && t < (MDA_sr + 1)) log(1/(1-MDA_eff1))*U[i,j,k,l] else 0
MDA_S[2:SMCyrs, 1:nh, 1:ni, 1] = if (t >= MDA_sr && t < (MDA_sr + 1)) log(1/(1-MDA_eff1))*S[i,j,k,l] else 0

MDA_A[2:SMCyrs, 1:nh, 1:ni, 2] = if (t >= MDA_sr && t < (MDA_sr + 1)) log(1/(1-MDA_eff2))*A[i,j,k,l] else 0
MDA_T[2:SMCyrs, 1:nh, 1:ni, 2] = if (t >= MDA_sr && t < (MDA_sr + 1)) log(1/(1-MDA_eff2))*T[i,j,k,l] else 0
MDA_D[2:SMCyrs, 1:nh, 1:ni, 2] = if (t >= MDA_sr && t < (MDA_sr + 1)) log(1/(1-MDA_eff2))*D[i,j,k,l] else 0
MDA_U[2:SMCyrs, 1:nh, 1:ni, 2] = if (t >= MDA_sr && t < (MDA_sr + 1)) log(1/(1-MDA_eff2))*U[i,j,k,l] else 0
MDA_S[2:SMCyrs, 1:nh, 1:ni, 2] = if (t >= MDA_sr && t < (MDA_sr + 1)) log(1/(1-MDA_eff2))*S[i,j,k,l] else 0
...
# S - SUSCEPTIBLE
init_S[,,,] <- user()
dim(init_S) <- c(na,nh,ni,ncc)
initial(S[,,,]) <- init_S[i,j,k,l]
dim(S) <- c(na,nh,ni,ncc)

deriv(S[1, 1:nh, 1:ni, 1:ncc]) <- -FOI[i,j,k,l]*S[i,j,k,l] + rP*P[i,j,k,l] + rU*U[i,j,k,l] +
...
deriv(S[2:na, 1:nh, 1:ni, 1:ncc]) <-  -FOI[i,j,k,l]*S[i,j,k,l] + rP*P[i,j,k,l] + rU*U[i,j,k,l] -
  (eta+age_rate[i])*S[i,j,k,l] + age_rate[i-1]*S[i-1,j,k,l]  - MDA_S[i,j,k,l]


# T- SUCCESSFULLY TREATED
init_T[,,,] <- user()
dim(init_T) <- c(na,nh,ni, ncc)
initial(T[,,,]) <- init_T[i,j,k,l]
dim(T) <- c(na,nh,ni,ncc)

deriv(T[1, 1:nh, 1:ni, 1:ncc]) <- ft*clin_inc[i,j,k,l] - rT*T[i,j,k,l] -
  (eta+age_rate[i])*T[i,j,k,l]  - MDA_T[i,j,k,l]

deriv(T[2:na, 1:nh, 1:ni, 1:ncc]) <- ft*clin_inc[i,j,k,l] - rT*T[i,j,k,l] -
  (eta+age_rate[i])*T[i,j,k,l] + age_rate[i-1]*T[i-1,j,k,l] - MDA_T[i,j,k,l]

# D - CLEAR DISEASE
init_D[,,,] <- user()
dim(init_D) <- c(na,nh,ni, ncc)
initial(D[,,,]) <- init_D[i,j,k,l]
dim(D) <- c(na,nh,ni,ncc)

deriv(D[1, 1:nh, 1:ni, 1:ncc]) <- (1-ft)*clin_inc[i,j,k,l] - rD*D[i,j,k,l] -
  (eta+age_rate[i])*D[i,j,k,l] - MDA_D[i,j,k,l]
deriv(D[2:na, 1:nh, 1:ni, 1:ncc]) <- (1-ft)*clin_inc[i,j,k,l] - rD*D[i,j,k,l] -
  (eta+age_rate[i])*D[i,j,k,l] + age_rate[i-1]*D[i-1,j,k,l] - MDA_D[i,j,k,l]

# A - ASYMPTOMATIC DISEASE
init_A[,,,] <- user()
dim(init_A) <- c(na,nh,ni,ncc)
initial(A[,,,]) <- init_A[i,j,k,l]
dim(A) <- c(na,nh,ni,ncc)

deriv(A[1, 1:nh, 1:ni, 1:ncc]) <- (1-phi[i,j,k,l])*FOI[i,j,k,l]*Y[i,j,k,l] - FOI[i,j,k,l]*A[i,j,k,l] +
  rD*D[i,j,k,l] - rA*A[i,j,k,l] - (eta+age_rate[i])*A[i,j,k,l] - MDA_A[i,j,k,l]
deriv(A[2:na, 1:nh, 1:ni, 1:ncc]) <- (1-phi[i,j,k,l])*FOI[i,j,k,l]*Y[i,j,k,l] - FOI[i,j,k,l]*A[i,j,k,l] +
  rD*D[i,j,k,l] - rA*A[i,j,k,l] - (eta+age_rate[i])*A[i,j,k,l] + age_rate[i-1]*A[i-1,j,k,l] - MDA_A[i,j,k,l]

# U - SUBPATENT DISEASE
init_U[,,,] <- user()
dim(init_U) <- c(na,nh,ni,ncc)
initial(U[,,,]) <- init_U[i,j,k,l]
dim(U) <- c(na,nh,ni,ncc)

deriv(U[1, 1:nh, 1:ni, 1:ncc]) <- rA*A[i,j,k,l] - FOI[i,j,k,l]*U[i,j,k,l] - rU*U[i,j,k,l] -
  (eta+age_rate[i])*U[i,j,k,l] - MDA_U[i,j,k,l]
deriv(U[2:na, 1:nh, 1:ni, 1:ncc]) <- rA*A[i,j,k,l] - FOI[i,j,k,l]*U[i,j,k,l] - rU*U[i,j,k,l] -
  (eta+age_rate[i])*U[i,j,k,l] + age_rate[i-1]*U[i-1,j,k,l] - MDA_U[i,j,k,l]

# P - PROPHYLAXIS
init_P[,,,] <- user()
dim(init_P) <- c(na,nh,ni,ncc)
initial(P[,,,]) <- init_P[i,j,k,l]
dim(P) <- c(na,nh,ni, ncc)

deriv(P[1, 1:nh, 1:ni, 1:ncc]) <- rT*T[i,j,k,l] - rP*P[i,j,k,l] - (eta+age_rate[i])*P[i,j,k,l] + MDA_T[i,j,k,l] + MDA_D[i,j,k,l] + MDA_A[i,j,k,l] + MDA_U[i,j,k,l] + MDA_S[i,j,k,l]
deriv(P[2:na, 1:nh, 1:ni, 1:ncc]) <- rT*T[i,j,k,l] - rP*P[i,j,k,l] - (eta+age_rate[i])*P[i,j,k,l] +
...
dim(Y) <- c(na,nh,ni, ncc)
Y[1:na, 1:nh, 1:ni, 1:ncc] <- S[i,j,k,l]+A[i,j,k,l]+U[i,j,k,l]

# The number of new cases at this timestep
dim(clin_inc) <- c(na,nh,ni, ncc)
clin_inc[1:na, 1:nh, 1:ni, 1:ncc] <- phi[i,j,k,l]*FOI[i,j,k,l]*Y[i,j,k,l]

dim(clin_inc_ACD) <- c(na,nh,ni, ncc)
clin_inc_ACD[1:na, 1:nh, 1:ni, 1:ncc] <- FOI[i,j,k,l]*Y[i,j,k,l]
...
dm <- user() # decay of maternal immunity
uc <- user() # scale parameter (see Supplementary mats. 3.1.2)
dc <- user() # decay for clinical immunity
db <- user() # decay for infection blocking immunity
ub <- user() # scale param for IB immunity
dd <- user() # decay for detection immunity
...
init_ICM[,,,] <- user()
dim(init_ICM) <- c(na,nh,ni,ncc)
initial(ICM[,,,]) <- init_ICM[i,j,k,l]
dim(ICM) <- c(na,nh,ni,ncc)

deriv(ICM[1, 1:nh, 1:ni, 1:ncc]) <- -1/dm*ICM[i,j,k,l] + (init_ICM[i,j,k,l]-ICM[i,j,k,l])/x_I[i]
deriv(ICM[2:na, 1:nh, 1:ni, 1:ncc]) <- -1/dm*ICM[i,j,k,l] - (ICM[i,j,k,l]-ICM[i,j,k,l])/x_I[i]

# ICA - exposure driven immunity
init_ICA[,,,] <- user()
dim(init_ICA) <- c(na,nh,ni,ncc)
initial(ICA[,,,]) <- init_ICA[i,j,k,l]
dim(ICA) <- c(na,nh,ni,ncc)

deriv(ICA[1, 1:nh, 1:ni, 1:ncc]) <- FOI[i,j,k,l]/(FOI[i,j,k,l] * uc + 1) - 1/dc*ICA[i,j,k,l] -ICA[i,j,k,l]/x_I[i]
deriv(ICA[2:na, 1:nh, 1:ni, 1:ncc]) <- FOI[i,j,k,l]/(FOI[i,j,k,l] * uc + 1) - 1/dc*ICA[i,j,k,l] - (ICA[i,j,k,l]-ICA[i-1,j,k,l])/x_I[i]

# clinical immunity is a combination of maternal and exposure-driven immunity
dim(IC) <- c(na,nh,ni,ncc)
...
dim(phi) <- c(na,nh,ni,ncc)
phi[1:na,1:nh,1:ni,1:ncc] <- phi0*((1-phi1)/(1+(IC[i,j,k,l]/IC0)^kc) + phi1)

# IB - infection blocking immunity
init_IB[,,,] <- user()
dim(init_IB) <- c(na,nh,ni, ncc)
initial(IB[,,,]) <- init_IB[i,j,k,l]
dim(IB) <- c(na,nh,ni,ncc)

deriv(IB[1, 1:nh, 1:ni, 1:ncc]) <- EIR[i,j,k,l]/(EIR[i,j,k,l]* ub + 1) - IB[i,j,k,l]/db - IB[i,j,k,l]/x_I[i]
deriv(IB[2:na, 1:nh, 1:ni, 1:ncc]) <- EIR[i,j,k,l]/(EIR[i,j,k,l]* ub + 1) - IB[i,j,k,l]/db - (IB[i,j,k,l]-IB[i-1,j,k,l])/x_I[i]

# b - probability of disease from infectious bite, depends on infection blocking immunity
bh <- user() # these parameters characterise the hill function for b
bmin <- user() # prob of infection from bite with zero immunity
kb <- user() #
IB0 <- user()
dim(b) <- c(na,nh,ni,ncc)
b[1:na, 1:nh, 1:ni, 1:ncc] <- bh * ((1-bmin)/(1+(IB[i,j,k,l]/IB0)^kb)+bmin)

# detection immunity
init_ID[,,,] <- user()
dim(init_ID) <- c(na,nh,ni,ncc)
initial(ID[,,,]) <- init_ID[i,j,k,l]
dim(ID) <- c(na,nh,ni,ncc)

deriv(ID[1, 1:nh, 1:ni, 1:ncc]) <- FOI[i,j,k,l]/(FOI[i,j,k,l]*ud + 1) - ID[i,j,k,l]/dd - ID[i,j,k,l]/x_I[i]
deriv(ID[2:na, 1:nh, 1:ni, 1:ncc]) <- FOI[i,j,k,l]/(FOI[i,j,k,l]*ud + 1) - ID[i,j,k,l]/dd - (ID[i,j,k,l]-ID[i-1,j,k,l])/x_I[i]
...
ad0 <- user() # no idea where these are from
...
dim(p_det) <- c(na,nh,ni,ncc)
p_det[,,,] <- dmin + (1-dmin)/(1 + fd[i]*(ID[i,j,k,l]/ID0)^kd)

# Force of infection, depends on level of infection blocking immunity
dim(FOI_lag) <- c(na,nh,ni,ncc)
FOI_lag[1:na, 1:nh, 1:ni, 1:ncc] <- EIR[i,j,k,l]/B2 * (if(IB[i,j,k,l]==0) bh else b[i,j,k,l])
...
cT <- user() # T -> mosq
g_inf <- user() # fitted value of g_inf characterises cA function
dim(cA) <- c(na,nh,ni,ncc)
cA[,,,] <- cU + (cD-cU)*p_det[i,j,k,l]^g_inf


# Force of infection from humans to mosquitoes
dim(FOIvijk) <- c(na,nh,ni,ncc)
omega <- user() #normalising constant for biting rates
FOIvijk[1:na, 1:nh, 1:ni, 1:ncc] <- (cT*T[i,j,k,l] + cD*D[i,j,k,l] + cA[i,j,k,l]*A[i,j,k,l] + cU*U[i,j,k,l]) * rel_foi[j] * av_mosq[k]*foi_age[i]/(omega*B2)
lag_FOIv=sum(FOIvijk)
# Current hum->mos FOI depends on the number of individuals now producing gametocytes (12 day lag)
latgam <- user() # latent period in humans
...
dl <- user() # development time of larvae
dp <- user() #development time of pupae
de <- user() #development time of early stage
mul <- user() #daily density dep. mortality rate of larvae
mup <- user() #daily den. dep. mortality rate of pupae
mue <- user() #daily den. dep. mortality rate of early stage
...
mu0 <- user() # daily hazard of death from external causes
...
ni <- user() # number of intervention categorys, ITN only, IRS only, neither, both
...
PHI_B <- user() # endophagy in bed
PHI_I <- user() # endophagy indoors
...
dim(av_mosq) <- ni
av_mosq[1:ni] <- av*B2*w[i]/wh # rate at which mosquitoes bite each int. cat.
dim(av_human) <- ni
av_human[1:ni] <- av*B2*yy[i]/wh # biting rate on humans in each int. cat.
...
dim(prev0to59) <- c(age59,nh,ni,ncc)
...
dim(prev2to10) <- c(age10,nh,ni,ncc)
...
dim(prev0to5_slide) <- c(age05,nh,ni,ncc)
...
dim(prev5to10_slide) <- c(age10,nh,ni,ncc)
...
dim(prev0to10_slide) <- c(age10,nh,ni,ncc)
