...
EIR[,,] <- av_human[k] * rel_foi[j] * foi_age[i] *(1-(IvFrac * v_effic_ALL))* Iv/omega
IvFrac <- Iv2/Iv
TRA_GSK <- user() #Based on the Johns Hopkins expts (~70%)
output(IvFrac) <- IvFrac

# TRA - TBA translation
#One distribution governing all infectious states
mALL <- 0.000157#0.4 #Negative binomial parameters
rrALL <- 0.00000495#0.01 #3.52 #Negative binomial parameters
...
deriv(Iv) <- Ev[10] - mu*Iv#incv - mu*Iv
...
########## Fake mosq. populations (larval sites)
# initial(Sv1) <- init_Sv * mv0
# initial(Ev1) <- init_Ev * mv0
# initial(Iv1) <- init_Iv * mv0
#
# ince1 <- FOIv * Sv1
# lag_incv1 <- ince1 * surv
# incv1 <- delay(lag_incv1, delayMos)
#
# deriv(Sv1) <- -ince1 - mu*Sv1 + betaa*(1-square*GSK_proportion)
# deriv(Ev1) <- ince1 - incv1 - mu*Ev1
# deriv(Iv1) <- incv1 - mu*Iv1
#
# initial(Sv2) <- 0 #init_Sv * mv0
# initial(Ev2) <- 0 #init_Ev * mv0
# initial(Iv2) <- 0 #init_Iv * mv0
#
# ince2 <- FOIv * Sv2
# lag_incv2 <- ince2 * surv
# incv2 <- delay(lag_incv2, delayMos)
#
# deriv(Sv2) <- -ince2 - mu*Sv2 + betaa*square*GSK_proportion
# deriv(Ev2) <- ince2 - incv2 - mu*Ev2
...
deriv(Ev1[1]) <- ince1 - Ev1[1] - mu*Ev1[1] #- muS*Ev1[1]*square
deriv(Ev1[2:10]) <- Ev1[i-1] - Ev1[i] - mu*Ev1[i] #- muS*Ev1[i]*square
mv1 = Sv1+sum(Ev1)+Iv1
...
initial(Ev2[1:10]) <- 0 # Options if not using a delayed delay
dim(Ev2) <- 10
...
deriv(Ev2[1]) <- ince2 - Ev2[1] - mu*Ev2[1] #+ muS*Ev2[1]*square
deriv(Ev2[2:10]) <- Ev2[i-1] - Ev2[i] - mu*Ev2[i] #+ muS*Ev2[i]*square
