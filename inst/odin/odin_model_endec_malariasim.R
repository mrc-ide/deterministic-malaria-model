...
deriv(Sv) <- -ince - mu_switch*Sv + betaa
deriv(Ev[1]) <- ince - Ev[1] - mu_switch*Ev[1]
deriv(Ev[2:10]) <- Ev[i-1] - Ev[i] - mu_switch*Ev[i]
deriv(Iv) <- Ev[10] - mu_switch*Iv
mv = Sv+sum(Ev)+Iv

mu_switch <- if (t >= endec_on && t <= endec_off) mu+mu_h else mu
