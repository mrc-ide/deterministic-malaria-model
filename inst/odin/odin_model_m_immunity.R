...
# dCM <- user() # decay of maternal immunity # no longer needed
...
# switched in the maternal immunity from the c++ version re: Patrick
dim(init_ICM_pre) <- c(nh,num_int)
init_ICM_pre[1:nh,1:num_int] <- PM*(ICA[age20l,i,j] + age_20_factor*(ICA[age20u,i,j]-ICA[age20l,i,j]))
ICM_age[]<-user()
dim(ICM_age)<-na
...
output(incunder5) <- sum(clin_inc0tounder5)/sum(den[1:age59]) # added
