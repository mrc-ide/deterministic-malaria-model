...
dim(EIR) <- c(na, nh, num_int)
EIR[,,] <- if (t < ITN_IRS_on) av_human[k] * rel_foi[j] * foi_age[i]/omega*Iv else
  av_human[k] * rel_foi[j] * foi_age[i]/omega*Iv_fixed
...
dim(av_mosq) <- num_int
# rate at which mosquitoes bite each int. cat.
av_mosq[1] <- if(t < ITN_IRS_on) av*w[i]/wh else 0.306667
av_mosq[2] <- av*w[i]/wh
dim(av_human) <- num_int
av_human[1] <- if(t < ITN_IRS_on) av*yy[i]/wh else 0.306667
av_human[2] <- av*yy[i]/wh # biting rate on humans in each int. cat.
...
dim(prev6to59) <- c(na,nh,num_int)
prev6to59[age6:age59,,] <- T[i,j,k] + D[i,j,k] + A[i,j,k]*p_det[i,j,k]
output(prev659) <- sum(prev6to59[,,])/sum(den[age6:age59])
output(prev659_1) <- sum(prev6to59[,,1])/(sum(den[age6:age59])*cov[1])
output(prev659_2) <- sum(prev6to59[,,2])/(sum(den[age6:age59])*cov[2])

dim(prev_all) <- c(na,nh,num_int)
prev_all[,,] <- T[i,j,k] + D[i,j,k] + A[i,j,k]*p_det[i,j,k]
output(prev_a) <- sum(prev_all[,,])/sum(den[])
output(prev_a_1) <- sum(prev_all[,,1])/(sum(den[])*cov[1])
output(prev_a_2) <- sum(prev_all[,,2])/(sum(den[])*cov[2])
...
output(inc_1) <- sum(clin_inc[,,1])
output(inc_2) <- sum(clin_inc[,,2])
...
output(EIR[]) <- TRUE
output(rel_foi) <- rel_foi
output(foi_age) <- foi_age
