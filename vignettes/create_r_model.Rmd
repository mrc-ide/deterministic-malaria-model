...
[vignette](https://mrc-ide.github.io/deterministic-malaria-model/articles/run_model.html).
...
1. Odin model path - `odin_model_path = system.file("extdata/odin_model.R", package = "ICDMM"`
...
```{r}
odin_model_path = system.file("extdata/odin_model_hrp2.R", package = "ICDMM")
```
...
# creates the odin model using the default odin model file
wh <- ICDMM:::create_r_model(odin_model_path = system.file("extdata/odin_model_hrp2.R", 
                                                              package = "ICDMM"))
mod <- wh$generator(user= wh$state, use_dde = TRUE)
```
...
an admin unit and country to the `ICDMM:::create_r_model()` function.  The latter is better when the admin 
